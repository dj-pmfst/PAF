...
class Projectile():
    ...
    def move_cube(self):
       loop = True
       while loop:
           self.area = self.a * ((self.a)/np.sin(np.arccos(self.vy[-1]/math.sqrt(self.vx[-1]**2 + self.vy[-1]**2))))
           self.R_K()
           if self.y[-1] <= 0:
               break 
        
    def angle_to_hit_target(self, x, y, r, v0, x0, y0, gustoca, m, povrsina, koeficijent, dt):
        kut = 0.1
        hit = False
        ...
        while hit == False:
            if kut < 90 :
                self.reset()
                self.kut = kut
                self.set_initial_conditions(v0, x0, y0, kut, gustoca, m, povrsina, koeficijent, dt)
                self.range()
                kut += 0.1
              ...
              #      hit = True
               # else:
                ...
                for i in range(len(self.x)):
                    if hit == False:
                        if (self.x[i] - x)**2 + (self.y[i] - y)**2 <= r**2:
                            hit = True
