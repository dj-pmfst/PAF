...
class ElectroMagnetism():
    def __init__(self, v0, polozaj, q, E, B, m, t):
        ...
        self.t = [0]
        self.t_uk = t
        self.a =[q*(E + np.cross(v0, self.__B_t(self.t[-1])))/m]
    ...
    def __acc(self, v, i):
        return self.q/self.m * (self.E + np.cross(v, self.__B_t(i)))
    ...
    def move_rk(self):
        ...
        while self.t[-1] <= self.t_uk:
            v1 = self.__acc(self.v[-1], self.t[-1]) * self.dt
            p1 = self.v[-1] * self.dt

            v2 = self.__acc(self.v[-1] + v1 / 2, self.t[-1]) * self.dt
            p2 = (self.v[-1] + p1 / 2) * self.dt

            v3 = self.__acc(self.v[-1] + v2 / 2, self.t[-1]) * self.dt
            p3 = (self.v[-1] + p2 / 2) * self.dt

            v4 = self.__acc(self.v[-1] + v3, self.t[-1]) * self.dt
